...
"""
COUNT_OPTION_2 = "samtools view test.bam | awk '{print length($10)}' | head -1000 | sort -u"
...
def combine_coverm(coverm_trimed, read_per_base, covered_fract_path, min_depth, untrimmed_output=False):
    ...
    if untrimmed_output:
        coverm_trimed.to_csv('coverm_trimmed_zeroed.tsv', sep='\t')
    else:
        keep = ~(coverm_trimed==0)
        coverm_trimed.loc[
            keep.any(axis=1), 
            keep.any(axis=0)].to_csv('coverm_trimmed_filtered.tsv', sep='\t')

def snakemake_run(data_path, out_path, threads, myparam):
    pass
