...
This pipeline is a distillation of the wealth of knowledge that the Wrighton lab has collected on read mapping distilled into an easy to use tool.

The motivation for this pipeline stems in part with the frustrations in other tools:

 - Should be able to work with single end reads, interleaved reads, and non interleaved reads (reformat to deinterleave)
 - To many useless files the only files that should be saved from this whole workflow are zipped BAMs and output tables and a log file.
 - Naming pains. In this pipeline the name is same as what the FASTQ files are named- and you can have standard naming for FASTQ (use coverM as example)


 The pipeline is based on Snakemake and benefits from Snakemakes tools.

How do I get it working
=======================

On the left of this documentation there is a menu, under the ""Get It Working heading you will find the minimal instructions that you need in order.

Go through the instructions in:

- :ref:`get_it_working-before_you_start`
- :ref:`get_it_working-configure_the_pipline`
- :ref:`get_it_working-run_the_pipline`


This should result in a fully successful run

.. _explain_the_pipeline:

Usage: Explaining the pipeline
===============================

If you know what you're doing, a TLDR of any pipeline is the input and output.
...
Below you can see a more complex overview of both the metaG and metaT pipelines with all available outputs.
Your run is guaranteed to be different from this pathway based on your config file. If you are new to Snakemake, see New to Snakemake Primer that will explain just enough Snakemake for you to get started with this pipeline.
...
.. toctree::
  :caption: Get it Working
  :name: getting_it_working
  :hidden:
  :maxdepth: 1

  get_it_working/before_you_start
  get_it_working/configure_the_pipline
  get_it_working/run_the_pipline
